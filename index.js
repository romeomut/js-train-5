...
for (const iterator of restElements) {
  if(minv > iterator || minv == 0){
    minv = iterator
  }
  if(maxv < iterator || maxv == 0){
    maxv = iterator
  } 
}
...
function printStrings(array) {
  ...
  let str = array.toString() + array.toString()
  console.log(str)
}
...
function swapMinMax(array) {
  ...
  // Ініціалізуємо дві змінні (  let maxIndex = 0 і minIndex) з нульового значення. Ці змінні будуть використовуватися для зберігання індексів максимального та мінімального значень в масиві.
...
function replaceNumbers(arr) {
  ...
  for (const key in arr) {
    newarr[key] =  arr[key] <= 10 ? "Less than or equal to 10" : "Greater than 10"
  }
